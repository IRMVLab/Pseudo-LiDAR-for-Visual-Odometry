...
__global__ void fused_conv_select_k_gpu(int batch_size, int H, int W, int npoints, int kernel_size_H, 
	...
	{		
		...
		for (int current_HW_idx = 0; current_HW_idx < kernel_total; ++current_HW_idx) //select points in every kernel element
		{
			...
			if ( (kernel_select_H_idx < 0) || (kernel_select_H_idx >= small_h) || (kernel_select_W_idx < 0) || (kernel_select_W_idx >= small_w)) //  the region of padding points (not valid)
			{
				++m_idx;
				continue;
			}
...
void FusedConvSelectKLauncher(int batch_size, int H, int W, int npoints, int kernel_size_H, 
	...
	fused_conv_select_k_gpu<<<batch_size,256,0,stream>>>(batch_size, H, W, npoints, kernel_size_H, kernel_size_W, K, flag_copy, distance, stride_h, stride_w, xyz1, xyz2, idx_n2, random_hw, selected_b_idx, selected_h_idx, selected_w_idx, valid_idx, valid_in_dis_idx, selected_mask, small_h, small_w);
