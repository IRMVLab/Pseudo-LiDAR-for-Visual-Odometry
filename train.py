...
os.system('cp %s %s' % ('pwclonet_model_utils.py', log_dir))
'''LOG'''
...
def main():
    ...
    train_dir_list = [0,1,2,3,4,5,6,7,8]#[0,2,8,9]#
    test_dir_list = [9,10]#[3,4,5,6,7,10]#
    ...
    for epoch in range(init_epoch + 1, args.max_epoch):
        ...
        for i, (batch,T_trans,T_trans_inv,T_gt,Tr) in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            
            pos1 = batch['pose'][:args.batch_size]
            pos2 = batch['pose'][args.batch_size:]
            calib1 = batch['calib'][:args.batch_size]
            calib2 = batch['calib'][args.batch_size:]
            xy1 = batch['xy'][:args.batch_size]
            xy2 = batch['xy'][args.batch_size:]
            image1 = batch['image'][:args.batch_size]
            image2 = batch['image'][args.batch_size:]

            pos1 = torch.tensor(pos1, device='cuda')
            pos2 = torch.tensor(pos2, device='cuda')
            xy1 = torch.tensor(xy1, device='cuda').to(torch.float32)
            xy2 = torch.tensor(xy2, device='cuda').to(torch.float32)
            image1 = torch.tensor(image1, device='cuda').to(torch.float32)
            image2 = torch.tensor(image2, device='cuda').to(torch.float32)
            T_trans = torch.tensor(T_trans, device='cuda').to(torch.float32)
            T_trans_inv = torch.tensor(T_trans_inv, device='cuda').to(torch.float32)
            T_gt = torch.tensor(T_gt, device='cuda').to(torch.float32)

            model = model.train()

            l0_q, l0_t, l1_q, l1_t, l2_q, l2_t, l3_q, l3_t, pc1_ouput, q_gt, t_gt, w_x, w_q = model(image2, image1, xy2, xy1,pos2, pos1, calib2, calib1, T_trans, T_trans_inv, T_gt)
...
def eval_pose(model, test_list, epoch):
    for item in test_list:
        ...
        for batch_id, (batch,T_trans,T_trans_inv,T_gt,Tr) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):

            pos1 = batch['pose'][:args.eval_batch_size]
            pos2 = batch['pose'][args.eval_batch_size:]
            calib1 = batch['calib'][:args.eval_batch_size]
            calib2 = batch['calib'][args.eval_batch_size:]
            xy1 = batch['xy'][:args.batch_size]
            xy2 = batch['xy'][args.batch_size:]
            image1 = batch['image'][:args.batch_size]
            image2 = batch['image'][args.batch_size:]
