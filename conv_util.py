...
class PointNetSaModule(nn.Module):
    ...
    def forward(self, xyz_proj, points_proj, xyz_sampled_proj):
        ...
        random_HW = (torch.arange(self.kernel_size[0] * self.kernel_size[1])).cuda().int()
        ...
        with torch.no_grad():
            ...
            fused_conv_select_k(xyz_proj.contiguous(), xyz_proj.contiguous(), idx_n2_part, random_HW, H, W, n_sampled, self.kernel_size[0], self.kernel_size[1], self.K_sample, 0, self.distance,\
                1, 1, select_b_idx, select_h_idx, select_w_idx, valid_idx, valid_in_dis_idx, valid_mask, H, W)
            
        uv = torch.cat([select_h_idx,select_w_idx],3)
    ...
    def forward(self, warped_xyz1_proj, xyz2_proj, points1_proj, points2_proj):
        ...
        # Sample QNN of (M neighbour points from sampled n points in PC1) in PC2
            select_b_idx, select_h_idx, select_w_idx, valid_idx2, valid_in_dis_idx2, valid_mask = fused_conv_select_k\
                ( warped_xyz1_proj.contiguous(), xyz2_proj.contiguous(), idx_hw, random_HW_q, H, W, H * W, self.kernel_size2[0], self.kernel_size2[1],\
    ...
    ##########################################################################################################################
        ...
        # # Sample QNN of (M neighbour points from sampled n points in PC1) in PC1
            select_b_idx, select_h_idx, select_w_idx, valid_idx2, valid_in_dis_idx2, valid_mask2 = \
                fused_conv_select_k( warped_xyz1_proj.contiguous(), warped_xyz1_proj.contiguous(), idx_hw, random_HW_p, H, W, H * W, self.kernel_size1[0], self.kernel_size1[1], 
...
class set_upconv_module(nn.Module):
    ...
    def forward(self, xyz1_proj, xyz2_proj, points1_proj, feat2_proj):
        ...
        # output the KNN of n dense points in n' sparse points (skip connection) 
            xyz1_up_xyz_points_b_idx, xyz1_up_xyz_points_h_idx, xyz1_up_xyz_points_w_idx, valid_idx, valid_in_dis_idx, valid_mask = fused_conv_select_k(
                xyz1_proj.contiguous(), xyz2_proj.contiguous(), idx_hw, random_HW, H, W, H*W, 
